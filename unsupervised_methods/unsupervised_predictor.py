...
def save_outputs(method_name, predictions, labels, config):

    output_dir = config.UNSUPERVISED.OUTPUT_SAVE_DIR
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    
    output_path = os.path.join(output_dir, method_name + '_outputs.pickle')
...
def unsupervised_predict(config, data_loader, method_name):
    ...
    for _, test_batch in enumerate(sbar):
        ...
        for idx in range(batch_size):
            ...
            predictions[subj_index][sort_index] = BVP[idx * chunk_len:(idx + 1) * chunk_len]
            labels[subj_index][sort_index] = labels_input[idx * chunk_len:(idx + 1) * chunk_len]
    ...
    if config.UNSUPERVISED.OUTPUT_SAVE_DIR: # saving test outputs
        save_outputs(method_name, predictions, labels, config)
