...
def _calculate_peak_hr(ppg_signal, fs, height=None, distance=None, prominence=None):
    """Calculate heart rate based on PPG using peak detection."""
    ppg_peaks, _ = scipy.signal.find_peaks(ppg_signal, height=height, distance=distance, prominence=prominence)
...
def calculate_metric_per_video(predictions, labels, gt_hr=None, fs=30, diff_flag=True, use_bandpass=True, hr_method='FFT', distance=None, height=None, prominence=None):
    ...
    if hr_method == 'FFT':
        ...
        hr_pred = _calculate_peak_hr(predictions, fs=fs, height=height, distance=distance, prominence=prominence)
        hr_label = _calculate_peak_hr(labels, fs=fs, height=height, distance=distance, prominence=prominence)
    else:
        raise ValueError('Please use FFT or Peak to calculate your HR.')

    if gt_hr is not None:
        SNR = _calculate_SNR(predictions, gt_hr, fs=fs)
    else:
