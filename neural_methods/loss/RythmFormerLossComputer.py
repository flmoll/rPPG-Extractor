...
class RhythmFormer_Loss(nn.Module): 
    ...
    def forward(self, pred_ppg, labels , epoch , FS , diff_flag):    
        ...
        loss_CE , loss_distribution_kl = TorchLossComputer.Frequency_loss(pred_ppg.squeeze(-1),  labels.squeeze(-1), diff_flag=diff_flag, Fs=FS, std=3.0, device=self.device)
        loss_hr = TorchLossComputer.HR_loss(pred_ppg.squeeze(-1),  labels.squeeze(-1), diff_flag=diff_flag, Fs=FS, std=3.0, device=self.device)
...
class TorchLossComputer(object):
    @staticmethod
    def compute_complex_absolute_given_k(output, k, N, device='cuda'):
        ...
        output = output.view(1, 1, -1).to(device=device, dtype=torch.float)
        k = k.view(1, -1, 1)
    ...
    @staticmethod
    def complex_absolute(output, Fs, bpm_range=None, device='cuda'):
        ...
        complex_absolute = TorchLossComputer.compute_complex_absolute_given_k(output, k, N, device=device)

        return (1.0 / complex_absolute.sum()) * complex_absolute	# Analogous Softmax operator


    @staticmethod
    def cross_entropy_power_spectrum_loss(inputs, target, Fs, device='cuda'):
        inputs = inputs.view(1, -1)
        target = target.view(1, -1)
        bpm_range = torch.arange(40, 180, dtype=torch.float).to(device)
        #bpm_range = torch.arange(40, 260, dtype=torch.float).to(device)

        complex_absolute = TorchLossComputer.complex_absolute(inputs, Fs, bpm_range, device=device)
    ...
    @staticmethod
    def cross_entropy_power_spectrum_focal_loss(inputs, target, Fs, gamma, device='cuda'):
        inputs = inputs.view(1, -1)
        target = target.view(1, -1)
        bpm_range = torch.arange(40, 180, dtype=torch.float).to(device)
        #bpm_range = torch.arange(40, 260, dtype=torch.float).to(device)
    ...
    @staticmethod
    def cross_entropy_power_spectrum_forward_pred(inputs, Fs, device='cuda'):
        inputs = inputs.view(1, -1)
        bpm_range = torch.arange(40, 190, dtype=torch.float).to(device)
        #bpm_range = torch.arange(40, 180, dtype=torch.float).to(device)
        #bpm_range = torch.arange(40, 260, dtype=torch.float).to(device)
    ...
    @staticmethod
    def Frequency_loss(inputs, target, diff_flag , Fs, std, device='cuda'):
        ...
        ca = TorchLossComputer.complex_absolute(inputs, Fs, bpm_range, device=device)
        sa = ca/torch.sum(ca)
        ...
        return F.cross_entropy(ca.cpu(), hr_gt.cpu()) , kl_loss(sa , target_distribution)

    @staticmethod
    def HR_loss(inputs, target,  diff_flag , Fs, std, device='cuda'):
