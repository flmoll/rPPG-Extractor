""" 
This duplicates the PhysNet architecture but adds a quantile regression module.
...
class PhysNetQuantileRegressionModule(nn.Module):
    def __init__(self, frames=128):
        super(PhysNetQuantileRegressionModule, self).__init__()
        self.frames = frames
...
class PhysNet_Quantile(nn.Module):
    def __init__(self, frames=128, input_size=(72, 72)):
        super(PhysNet_Quantile, self).__init__()
        ...
        self.quantile_regression_module = PhysNetQuantileRegressionModule(frames=frames)
    ...
    def forward(self, x):  # Batch_size*[3, T, 128,128]
        ...
        quantile_upper = self.quantile_regression_module(x)
        ...
        quantile_lower = x.view(-1, length)

        mean = (torch.mean(quantile_lower, axis=1, keepdim=True) + torch.mean(quantile_upper, axis=1, keepdim=True)) / 2
        std = (torch.std(quantile_lower, axis=1, keepdim=True) + torch.std(quantile_upper, axis=1, keepdim=True)) / 2
        quantile_lower = (quantile_lower - mean) / std
        quantile_upper = (quantile_upper - mean) / std
