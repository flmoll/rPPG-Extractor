...
class PhysnetTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "only_test":
            if config.TEST.DATA.PREPROCESS.CONVERT_TO_TYPE != "DiffNormalized" and config.TEST.DATA.PREPROCESS.CONVERT_TO_TYPE != "Standardized":
                in_channels = 6
        else:
            if config.TRAIN.DATA.PREPROCESS.CONVERT_TO_TYPE != "DiffNormalized" and config.TRAIN.DATA.PREPROCESS.CONVERT_TO_TYPE != "Standardized":
                in_channels = 6

        self.model = PhysNet_padding_Encoder_Decoder_MAX(
            frames=config.MODEL.PHYSNET.FRAME_NUM, in_channels=in_channels).to(self.device)  # [3, T, 128,128]
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                rPPG, x_visual, x_visual3232, x_visual1616 = self.model(data.to(torch.float32))
                BVP_label = labels.to(torch.float32).to(self.device)
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                data, BVP_label = data_dict["data"].to(self.device), data_dict["label"].to(self.device)
                rPPG, x_visual, x_visual3232, x_visual1616 = self.model(
                    data.to(torch.float32).to(self.device))
                rPPG = (rPPG - torch.mean(rPPG)) / torch.std(rPPG)  # normalize
                BVP_label = (BVP_label - torch.mean(BVP_label)) / \
                            torch.std(BVP_label)  # normalize
