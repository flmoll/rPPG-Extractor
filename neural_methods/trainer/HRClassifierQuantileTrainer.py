...
from neural_methods.loss.HRClassifierQuantileLoss import HRClassifierQuantileLoss


class HRClassifierQuantileTrainer(BaseTrainer):
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                lower, upper = self.model(model_input)

                loss = self.loss_model(lower, upper, gt_hr)
                loss.backward()
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                hr_pred = (lower + upper) / 2  # Average of lower and upper bounds
                abs_errors_hr.extend(torch.abs(240 * (hr_pred - gt_hr)).cpu().numpy().tolist())

                loss_ecg = self.loss_model(lower, upper, gt_hr)
                valid_loss.append(loss_ecg.item())
                valid_step += 1
                vbar.set_postfix(loss=loss_ecg.item())
            valid_loss = np.asarray(valid_loss)
            print("HR errors: ", np.mean(abs_errors_hr))
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                heart_rates = (lower + upper) / 2
                heart_rates_uncertainty = (upper - lower) / 2  # Uncertainty is half the width of the interval
