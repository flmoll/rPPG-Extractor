...
from neural_methods.loss.PhysNetQuantileLoss import PhysNetQuantileRegressionLoss
from neural_methods.model.PhysNetQuantile import PhysNet_Quantile
from tqdm import tqdm



class PhysnetQuantileTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            self.loss_model = PhysNetQuantileRegressionLoss()
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                quantile_lower, quantile_upper = self.model(
                    data.to(torch.float32))
                BVP_label = labels.to(torch.float32)

                loss = self.loss_model(quantile_lower, quantile_upper, BVP_label)
                loss.backward()
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                quantile_lower, quantile_upper = self.model(data)

                # In the test dataset we do a prediction and interval width calculation
                # Then the usual metrics calculation can be done
                pred_ppg_test = (quantile_lower + quantile_upper) / 2
                uncertainty = (quantile_upper - quantile_lower) / 2
