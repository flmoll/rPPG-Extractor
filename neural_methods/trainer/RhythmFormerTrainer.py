...
class RhythmFormerTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        device_ids = [config.DEVICE]#list(range(config.NUM_OF_GPU_TRAIN))
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            self.criterion = RhythmFormer_Loss(device=self.device)
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                data_valid, labels_valid = data_dict["data"].to(self.device), data_dict["label"].to(self.device)
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _, test_batch in enumerate(tqdm(data_loader['test'])):
                ...
                data_test, labels_test = data_dict["data"].to(self.device), data_dict["label"].to(self.device)
                batch_size = data_test.shape[0]
                print(f"Testing shape: {data_test.shape}, labels shape: {labels_test.shape}")
                ...
                for ib in range(batch_size):
                    subj_index = test_batch[1][ib]
                    sort_index = int(test_batch[2][ib])
